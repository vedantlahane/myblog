...
class BlogApiClient implements ApiClient {
  ...
  async savePost(postId: string): Promise<{ message: string }> {
    return this.request<{ message: string }>(`/api/users/posts/${postId}/save`, {
      method: 'POST',
    });
  }
  ...
  async getSavedPosts(): Promise<Post[]> {
    return this.request<Post[]>('/api/users/me/saved-posts');
  }
  ...
  async getCommentsByPost(postId: string): Promise<Comment[]> {
    return this.request<Comment[]>(`/api/comments/post/${postId}`);
  }

  async getCommentReplies(commentId: string): Promise<Comment[]> {
    return this.request<Comment[]>(`/api/comments/${commentId}/replies`);
  }
  ...
  async getPopularTags(): Promise<Tag[]> {
    return this.request<Tag[]>('/api/tags/popular');
  }
  ...
  async getPostsByTag(id: string): Promise<Post[]> {
    return this.request<Post[]>(`/api/tags/${id}/posts`);
  }
  ...
  async getUserCollections(): Promise<any[]> {
    return this.request<any[]>('/api/collections/user/my-collections');
  }
  ...
  async globalSearch(query: string): Promise<any> {
    return this.request(`/api/search/all?q=${encodeURIComponent(query)}`);
  }
}
